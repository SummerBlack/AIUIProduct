...
public class DownloadUtil {
	 ...
	 * @throws Exception
	 */
	public void download(UpdateDownloadListener downloadListener)
			throws Exception {
		...
		for (int i = 0; i < threadNum; i++) {
			...
			RandomAccessFile currentFile = new RandomAccessFile(targetFile,
					"rw");
			currentFile.seek(startPosition);
			threads[i] = new DownloadThread(startPosition, currentPartSize,
					currentFile);
			// 启动下载线程
			threads[i].start();
			LogHelper.d("启动线程" + i);
	...
	private class DownloadThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				while (!mStopRun && length < currentPartSize
