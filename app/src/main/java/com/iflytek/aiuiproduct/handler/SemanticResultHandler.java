...
public class SemanticResultHandler {
	 ...
	 * @return
	 */
	private static SemanticResult parseSemanticResult(JSONObject json) {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				case MSG_SEMANTIC_RESULT:
				{
					...
					//遍历mSubDisposers中所有子处理器，看看到底这个结果应该交给谁处理
					...
					//如果上述子处理器都不能处理这条语义，则交给mDefaultDisposer处理器处理
					//其实就是简单的播报answertext
					if(!isHandled){
	...
	private void disposeResult(SemanticResult result) {
		if (null != mResultHandler && null != result) {
			//如果该次语义结果的操作码不是"INSTRUCTION",则将结果保存到mLastActiveResult中
